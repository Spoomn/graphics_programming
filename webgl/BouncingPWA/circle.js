class Circle {
    ...
    constructor(xlow, xhigh, ylow, yhigh, color, size){ // make the circles inside these World Coordinates
        ...
        if (color){ // if a color is passed in, use it
            this.color = color;
        }
        else{ // otherwise, pick a random color from the list
	...
	const gravityMultiplier = 8;

        this.dx += gravityVector[0] * DT * gravityMultiplier;

        this.dy += gravityVector[1] * DT * gravityMultiplier;



        const airFriction = .9999

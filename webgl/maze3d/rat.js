...
class Rat{
    ...
    draw(gl, shaderProgram){
        ...
        drawTriangle3d(gl, shaderProgram, .3,0,this.TALLNESS, -.2,.2,this.TALLNESS, -.2,-.2,this.TALLNESS, color); 

        // draw fat rat belly
        drawCircle3d(gl, shaderProgram, -.2,0,this.TALLNESS, .2, color);
        ...
        // draw the rat's eye
        drawCircle3d(gl, shaderProgram, -.1, -.04,this.TALLNESS+.001, .06, [0,0,0,1]); // outline
        drawCircle3d(gl, shaderProgram, -.1, -.04,this.TALLNESS+.002, .05, [1,1,1,1]); // white
        drawCircle3d(gl, shaderProgram, -.07, -.04,this.TALLNESS+.003, .02, [0,0,0,1]); // pupil
        // second eye
        drawCircle3d(gl, shaderProgram, -.1, .04,this.TALLNESS+.001, .06, [0,0,0,1]); // outline
        drawCircle3d(gl, shaderProgram, -.1, .04,this.TALLNESS+.002, .05, [1,1,1,1]); // white
        drawCircle3d(gl, shaderProgram, -.07, .04,this.TALLNESS+.003, .02, [0,0,0,1]); // pupil
        ...
        // rat's nose triangle
        drawTriangle3d(gl, shaderProgram, .2,.025,this.TALLNESS+.001, .2,-.025,this.TALLNESS+.001, .3,0,this.TALLNESS+.001, [.99,.42,.75,1]);

        // rat's ears
        drawTriangle3d(gl, shaderProgram, -.2,.2,this.TALLNESS, -.4,.15,this.TALLNESS, -.4,.25,this.TALLNESS, color);
        drawTriangle3d(gl, shaderProgram, -.2,-.2,this.TALLNESS, -.4,-.15,this.TALLNESS, -.4,-.25,this.TALLNESS, color);
        drawTriangle3d(gl, shaderProgram, -.22,.2,this.TALLNESS+.001, -.39,.235,this.TALLNESS+.001, -.39,.175,this.TALLNESS+.001, [.99,.42,.75,1]);
        drawTriangle3d(gl, shaderProgram, -.22,-.2,this.TALLNESS+.001, -.39,-.235,this.TALLNESS+.001, -.39,-.175,this.TALLNESS+.001, [.99,.42,.75,1]);
